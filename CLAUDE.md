...
| COLOUR | Four signals and the form row's letters (amended 2026-09-24, form row ruled 2026-09-25): `audit.colour_law_faults` refuses a value colour on any selector but `.sig-clears`/`.sig-costs` (outline declarations only), `.sig-won`/`.sig-lost` (fill declarations only) and `.fmark.win`/`.fmark.loss` (`color` only) | `plant.py::plant_a_green_link`, `::plant_a_red_warning_border`, `::plant_a_fill_on_a_pick_that_only_clears`, `::plant_an_outline_on_a_pick_that_won`, `::plant_a_value_colour_on_a_form_streak` (a form mark filled) |
| RULING c | A prop tile wears no outline against a multiplier the app assumed: `audit.board_signal_faults` refuses `clears`/`costs` on a tile whose `multiple_source` was not read; the colour is earned in the entry rail against the multiple the operator typed | `plant.py::plant_an_outline_on_a_prop_against_an_assumed_multiple` |
| MOTION | The bar fills once, on load, by a transform, and never on an update: `audit.bar_fill_faults` refuses any other movement on `.pbar-fill`, and `audit.live_update_faults` refuses a live patch that reaches the bar or an arrival; `PRICE_SELECTORS` names every board class a price, chance, payout or score is drawn in, so none may transition; a one-shot keyframe (`pop`) may not loop | `plant.py::plant_a_bar_that_fills_on_an_update`, `::plant_a_score_that_animates`, `::plant_a_price_that_moves_when_it_changes` |
| LAW 4 | A My day chip that is filled is a settled pick and carries its badge: `audit.board_signal_faults` walks `board.my_day.entries` as it walks rows and tiles | `plant.py::plant_a_glow_without_its_badge` (the same walk) |
| THE GATE | A comment that eats the next rule: `audit.stray_comment_marker_faults` refuses a `*/` standing outside any comment once the stylesheet's comments are blanked, because the browser drops the following rule whole and says nothing | `plant.py::plant_a_comment_that_eats_a_rule` |
