...
| NO SILENT SKIP | Operator ruling of 2026-09-23 (GRIDIRON_REPAIR item 2, built 2026-09-26): "run.py:99 may never skip an untrained market silently -- a predict run with a skipped market fails by name, and the day strip shows it." `baseline.untrained_markets` is the one door for which market a run asks and cannot answer: every market in `config.active_markets` that is not held (a prop only when the run asks props) whose `load_fit` finds no activated fit or an active fit of another factor set, or whose active fit reads a factor the registry no longer computes for it (`assert_the_vector_carries_the_fit`, against every factor the market's vector would hold). `predict.predict_slate` carries that list on the run (`BlindRun.untrained`) and adds any question skipped the same way that the door did not foresee; `run.run_slate` raises `MarketNotTrained`, naming each market and why in plain words, after the markets that have a model are written, snapshotted and ranked, and `run_task` records the run failed with those words (the final pass too). `run.already_answered` no longer leaves such a market out of what a run expects (0768ce4's rule, retired by the ruling), so a rerun is never refused as answered over a market it never asked; `predict.already_written` still keeps a rerun from writing any question twice. `views.freshness` prints a stale line for each sport the record has forecast, or whose run has been recorded failed for want of a model (`tasks.failed_for_want_of_a_model`; the prover, 2026-09-26: a sport whose first run meets no model writes nothing, so a strip waiting for a forecast never spoke), through the same door ("NFL moneyline not forecast: no model is in use for it"), and `audit.freshness_faults` refuses a strip that leaves a market off. A retired or held market is not untrained | `plant.py::plant_an_untrained_market_in_the_active_set`, `::plant_a_rerun_refused_over_a_market_it_never_asked`, `::plant_an_untrained_market_the_strip_leaves_off`, `::plant_a_first_run_failed_by_name_off_the_strip`, `::plant_a_fit_reading_a_retired_factor`, `test_untrained.py` |
| CORRECTIONS REACH RECOMMENDATIONS | Operator ruling of 2026-09-23 (GRIDIRON_REPAIR item 3, built 2026-09-26): "recommend.py:324 reads the corrected probability, never the raw claim. Re-derive nothing retroactively (LAW 3); from the fix forward, every recommendation carries the correction that was current." `correction.shown_proposition` is the one door for a number stated from a claim's fixed proposition (the home side, the over): it turns the number to the side it favours (a half favours the proposition, as `baseline.stated_side`), corrects that confidence through `shown_claim` -- the fit's own domain; applied unturned, a nonzero intercept puts picks on the wrong side -- and turns it back, returning the number and no version while nothing is in force. "Current" is the ACTIVE correction for the category (sport, market type, forecaster) at the instant: a fit the holdout did not activate is never applied (C2's gate). `recommend.for_predictions` prices the side, the edge, the bar and the size from it, corrected by what is in force now before the game starts and by what was in force when the claim was written once it has (`recommend.correction_instant`, the one rule for the pick and the at-the-line sentence beside it: the prover of 2026-09-26 found a finished game's pick turned by a correction activated after the game, while the sentence kept the raw claim); `record_for` writes the raw claim's number in `fair_value` as on every earlier row, and beside it `calibrated_fair_value` and `correction_version` (both NULL, together, when nothing was in force -- a CHECK pairs them), looked up before the row is stamped; `recommendation_correction_is_frozen` refuses an edit of either (a rule of its own: `recommendations_no_update`'s text is never replaced on a record that holds it). `db.MIGRATIONS` adds the two columns to an older record with exactly `schema.sql`'s clauses (`recommendations` is not a quarantined table, and since 5b the gate compares a copy `db.init` alone migrated). No earlier row is touched. The card's model chip, the recommendation line and a combo's legs read the entry's corrected `fair_value`; the at-the-line sentence reads the claim through the same door (now before the start, the claim's own instant after it), and a live card's pregame figure the claim corrected as of its writing. The ranker still orders on the raw claim (the brief forbids ranker changes; FOLLOWUPS) | `plant.py::plant_a_correction_that_does_not_reach_the_pick`, `test_recommend.py::test_an_active_correction_that_would_flip_the_side_flips_the_pick`, `::test_the_row_carries_the_correction_that_was_current_and_keeps_it`, `::test_a_finished_games_pick_is_never_re_derived_by_a_later_correction`, `::test_the_size_is_computed_from_the_corrected_claim`, `::test_a_correction_reaches_only_its_own_forecasters_picks`, `::test_the_card_the_line_sentence_and_the_pregame_figure_read_one_number`, `test_correction.py::test_a_proposition_is_turned_to_its_favoured_side_corrected_and_turned_back`, `test_schema.py::test_an_older_record_gains_the_correction_columns_exactly_as_declared` |
| THE COST OF THE SIDE | Operator ruling of 2026-09-23 (GRIDIRON_REPAIR item 4, built 2026-09-26): "Return-on-stake denominator: a no-side edge divides by the no-side cost. Re-grade the three recommendations it let through as 'would not have cleared'. Planting." `recommend.return_on_stake` and `recommend.clears_the_bar` take the side as a keyword with NO DEFAULT and divide the edge by what that side costs -- `recommend._cost_of`, the one orientation `edge_cents` shares: the yes price on the yes side, the rest of the dollar on the no side -- so a caller that forgets the side is a TypeError, never a yes price, and a pick with no side has no return. `for_predictions` asks the bar with the side the edge is on. Until then the bar divided every edge by the yes price: below a 50c yes price a no-side edge read larger than it was (recs 3, 10 and 26 cleared at 5.57%, 6.13% and 7.19% of the yes price and are 3.34%, 3.68% and 4.69% of what the no side cost; rec 56, written 24 September, 4.97%), and above 50c a real one was refused. THE RE-GRADE IS A LABEL, NEVER AN EDIT (LAW 3): `recommendation_regrades` holds one row beside a recommendation that stays as written -- the verdict "would not have cleared", what the side cost, the return on it and on the yes price, the bar, a reason -- and its triggers refuse a label the row's own frozen side, price and edge do not support, one stamped at or before its recommendation, an edit, a delete and a replacing insert. AND ONLY WHAT THE DIVISOR LET THROUGH (the prover, 2026-09-26: the rule took the bar from the label and admitted any row, so a pick that cleared 5.6% of its cost was taken labelled against an 8% bar, a pick the bar rounds to 5.0% and clears labelled 4.99%, one the yes price never passed, and one written before the bar): the label now needs a no-side row (the rule reads the no side's cost off the row) written on or after 2026-09-07, the declared 5% (pinned in the trigger as the activation gate's birthday is, and tested equal to `config.MIN_RETURN_ON_STAKE` and its date), and the row's own returns, rounded to the bar's four places, passed on the yes price and under the bar on its own cost. It takes nothing out of any count (the ruling says re-grade, not void): `calibration.clv_report` names it beside the closing line with its N ("Would not have cleared"). `tools/regrade_return_on_stake.py` selects by rule through the bar itself (`recommend.let_through_by_the_yes_price`, from the frozen row, never the outcome), is refused on the live record without `--live` (`db.is_the_live_record_file`), and writes only when the selection is exactly the ruled set, in one transaction, idempotently -- on 2026-09-26 it selects 3, 10, 26 and 56 against the ruling's "three", so it refuses until the operator rules (`docs/REPAIR_STATE.md` question 9). The no-side SIZE has the same shape and is not named (question 10) | `plant.py::plant_a_no_side_edge_divided_by_the_yes_price`, `::plant_a_regrade_that_is_false_or_rewritten`, `test_recommend.py::test_a_no_side_edge_is_a_share_of_what_the_no_side_costs`, `::test_above_fifty_cents_a_real_no_side_edge_now_clears`, `::test_the_bar_cannot_be_asked_without_naming_the_side`, `::test_rec_3s_numbers_are_not_recommended_and_the_mirror_is`, `::test_the_regrade_is_chosen_by_rule_from_the_frozen_row`, `::test_a_regrade_is_true_and_permanent`, `::test_a_regrade_labels_only_what_the_yes_price_let_through`, `::test_the_regrade_rule_pins_the_bar_as_declared`, `::test_the_closing_line_names_a_regrade_and_counts_it_where_it_was`, `::test_the_tool_refuses_a_selection_the_ruling_did_not_name` |
| ONE PER GAME AND MARKET | Operator ruling of 2026-09-23 (GRIDIRON_REPAIR item 5, built 2026-09-26): "One recommendation per game and market, and never both sides. Recs 45 and 46 are the planting." `recommend.one_per_game_and_market` is the one door for what a pass may write, and `record_for` asks it about the pass's picks before anything is written: none on a game and market that already hold a standing recommendation (`recommend.standing_recommendations`, first by stamp then id, through `not_withdrawn`), on either side, from any later pass or forecaster; NEITHER where one pass takes both sides of one game and market -- two forecasters or two rungs -- the conservative default, since the ruling names no survivor; and where several take one side, the forecast written first (the statistical row of a question is written before the reasoning row). "Market" is the table's own `market` (the prop type for a prop), whatever the rung or player: the literal and stricter reading. Each pick not written is counted by name (`already`, `second_on_game_market`, `both_sides`) and said in words in `refused`, which the predict and final tasks keep in the run's payload -- and `tasks.run_task` keeps on a run recorded failed by `run.MarketNotTrained`, which is raised after the run has recommended (the prover, 2026-09-26: such a run kept only its traceback). The schema's `recommendation_one_per_game_and_market` refuses any insert on a game and market holding a standing recommendation, however written, and `record_for` counts that refusal by the ruling's words (`recommend.ONE_PER_GAME_AND_MARKET`), never as `already`, a forecast written twice. A withdrawn recommendation does not stand, so NFL spreads 73, 75, 76 and 78 after 62, 63, 64 and 66 are lawful. NEW ROWS ONLY (LAW 3): the rule is made over the game-market pairs written before it -- eighteen standing, 36 rows, 45 and 46 among them, measured read-only on 2026-09-26 -- without reading them; they stand and are counted as written, none relabelled -- whether they should count once is `docs/REPAIR_STATE.md` question 12, and whether the page follows the record, question 11 (until ruled, the rule binds what is written) | `plant.py::plant_both_sides_of_one_total_recommended`, `test_recommend.py::test_one_pass_taking_both_sides_of_a_game_and_market_recommends_neither`, `::test_a_game_and_market_with_a_standing_recommendation_gets_no_second`, `::test_one_sides_picks_in_one_pass_are_recommended_once_from_the_first`, `::test_another_game_or_another_market_is_its_own`, `::test_the_schema_refuses_a_second_standing_recommendation_on_either_side`, `::test_the_rule_reads_no_row_already_written`, `::test_a_refusal_by_the_record_is_counted_by_its_own_name`, `::test_the_run_keeps_what_it_refused_and_why`, `::test_a_run_failed_for_want_of_a_model_keeps_what_it_recommended` |
