...
| FIRST SCREEN | A job that fails is visible on the first screen the operator opens: the day strip prints the age of the last successful daily run, venue read and reasoning pass, marked stale past `config.FRESHNESS_HOURS` (dated) with the threshold in the words, and `audit.check_the_strip_shows_a_dead_job` refuses a payload that hides a stale job or calls one fresh. Not red: the colour law keeps red for a loss, and that fork awaits a ruling | `plant.py::plant_a_dead_job_the_strip_calls_fresh` |
