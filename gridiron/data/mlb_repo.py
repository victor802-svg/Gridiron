...
def game_date(conn: sqlite3.Connection, game_id: str) -> str | None:
    row = conn.execute(
        "SELECT game_date FROM mlb_team_games WHERE game_id = ? LIMIT 1", (game_id,)
    ).fetchone()
    if row:
        return row["game_date"]
    row = conn.execute(
        "SELECT kickoff_utc FROM games WHERE id = ?", (game_id,)
    ).fetchone()
    return row["kickoff_utc"][:10] if row and row["kickoff_utc"] else None
