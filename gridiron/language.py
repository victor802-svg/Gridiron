...
def call_side_label(item: dict, side: str) -> str:
    """What one side of this question is called, for a button.

    THROUGH `side_named`, like every other naming of a side on this page. The
    buttons are the one place a person chooses a side rather than reads one,
    which makes them the worst possible place for a second vocabulary: a
    button reading "not_cover" would be the stored identifier reaching the
    interface, and a button that named the wrong team would record the
    opposite of what was clicked.
    """
    shown = dict(item)
    shown["model_side"] = side
    named, _ = side_named(shown, form="city")
    market = item.get("market_type")
    if market == "total":
        # THE NUMBER DOES NOT FLIP ON A TOTAL. Both sides are asked at the
        # same number -- the under of 51.5 IS 51.5. `flipped_line` negates for
        # a no-side because a spread's other team RECEIVES the points, which
        # is true of a spread and nonsense here: the first version of this
        # button read "the under -51.5".
        return f"{named} {_number(item.get('line_asked'))}".strip()
    if market == "moneyline":
        return f"{named} to win"
    if market == "spread":
        line = flipped_line(shown)
        return f"{named} {line:+.1f}" if line is not None else named
    return f"{named} {side_word_or_side(side)}".strip()


def call_state_line(call: dict | None, started: bool,
                    outcome: int | None = None) -> str:
    """What the call block says about where the call stands.

    Four states and each says something different: none yet, made and still
    changeable, locked at kickoff, and settled. The locked wording names WHEN
    it was locked -- "called before kickoff" -- because that is the fact that
    makes the call a forecast rather than a report, and it is the one a reader
    will want when looking back at a win.
    """
    if call is None:
        return "no call on this one" if started else "your call"
    tier = call.get("tier", "")
    if outcome is not None:
        return f"you called {tier}"
    if started:
        return f"{tier} · called before kickoff"
    return f"{tier} · you can still revise this"


def call_comparison_line(n: int, you_right: int, model_right: int,
                         market_right: int | None, gate: int) -> str | None:
    """"On the 24 picks you called, you were right 58%, the model 54%."

    None below the gate, and that is LAW 4 rather than modesty: a hit rate on
    six calls is the most persuasive misleading number this page could show,
    because it is about the reader themselves.

    The market's figure is included only when every one of those picks had a
    line; a market percentage computed over the subset that happened to be
    priced would be a different set of games wearing the same denominator.
    """
    if n < gate:
        return None
    parts = [f"On the {n} picks you called, you were right "
             f"{round(you_right / n * 100)}%",
             f"the model {round(model_right / n * 100)}%"]
    if market_right is not None:
        parts.append(f"the market {round(market_right / n * 100)}%")
    return ", ".join(parts) + "."


def calls_since_line(right: int, wrong: int) -> str | None:
    """"your calls: 3 right, 2 wrong". None when nothing of yours settled."""
    if not (right or wrong):
        return None
    return f"your calls: {right} right, {wrong} wrong"
