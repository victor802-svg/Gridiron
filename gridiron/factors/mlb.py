...
@factor(
    ...
    note=(
        ...
        "The reason is the shape of the question. Every MLB question asks "
        "whether the HOME club wins, so the home side is home in one hundred "
        "percent of rows and this factor returns 1.0 every time. A column that "
        "never varies is absorbed entirely into the intercept, which is where "
        "the effect actually lives: the fitted intercept of 0.0913 puts a "
        "league-average home club at 52.3% - baseball's home edge, measured "
        "rather than assumed, and smaller than football's as expected. "
        "NFL's home_away varies because its subject rotates between the two "
        "clubs across the spread ladder; a moneyline has no ladder to rotate "
        "on, and rotating the subject instead would ask the exact complement "
        "question and double-count every game. So there is no repair to make "
        "here and none is offered - the quantity is measured, just not by a "
        "factor. History stays; the deactivation is recorded rather than the "
        "declaration being deleted."
    ),
    rationale=(
